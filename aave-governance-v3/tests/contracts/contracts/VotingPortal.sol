...
contract VotingPortal is Ownable, IVotingPortal {
  ...
  // stores the gas limit for vote via portal bridging tx
  uint128 internal _voteViaPortalGasLimit;
   ...
   * @param voteViaPortalGasLimit gas limit for "Vote via portal" bridging tx
   */
  constructor(
    ...
    _updateVoteViaPortalGasLimit(voteViaPortalGasLimit);
  ...
  function didVoterVoteOnProposal(
    ...
    return _proposalVoters[proposalId][voter];
  }
  ...
  function forwardVoteMessage(
    ...
    require(
      !didVoterVoteOnProposal(proposalId, voter),
      Errors.VOTER_ALREADY_VOTED_ON_PROPOSAL
    );
    ...
    _proposalVoters[proposalId][voter] = true;

    _sendMessage(
      msg.sender,
      MessageType.Vote,
      getVoteViaPortalGasLimit(),
      message
    );
  }
  ...
  /// @inheritdoc IVotingPortal
  function setVoteViaPortalGasLimit(uint128 gasLimit) external onlyOwner {
    _updateVoteViaPortalGasLimit(gasLimit);
  }
  ...
  /// @inheritdoc IVotingPortal
  function getVoteViaPortalGasLimit() public view returns (uint128) {
    return _voteViaPortalGasLimit;
  }
  ...
  /**
   * @notice method to update the _voteViaPortalGasLimit
   * @param gasLimit the new gas limit
   */
  function _updateVoteViaPortalGasLimit(uint128 gasLimit) internal {
    _voteViaPortalGasLimit = gasLimit;
    emit VoteViaPortalGasLimitUpdated(gasLimit);
  }
}
